...
ngDomAccessProvider.prototype.focusCellElement = function ($scope, index) { 
    if ($scope.selectionProvider.lastClickedRow) {
        ...
        if (columnIndex !== undefined && elm) {
            var columns = elm[0].children; // angular.element(elm[0].children).filter(function () { return this.nodeType !== 8; }); //Remove html comments for IE8
    ...
    self.setDraggables = function() {
        if (!grid.config.jqueryUIDraggable) {
            ...
            angular.forEach(columns, function(col){
                if(col && col.className && col.className.indexOf("ngHeaderSortColumn") !== -1){
